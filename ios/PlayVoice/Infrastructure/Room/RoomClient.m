...
-(void)join {
    ...
    NSString *name = self.displayName ?: [NSString stringWithFormat:@"%lld", self.currentUID];
    NSDictionary *data = @{
        @"displayName": name,
        @"device": device,
        @"produceVideo": @(TRUE),
    ...
    [self request:@"join" jsonData:data
        ...
        if ([self.delegate respondsToSelector:@selector(roomClient:didJoinWithPeers:)]) {
            [self.delegate roomClient:self didJoinWithPeers:peers];
        }
...
-(void)produceVideo {
    ...
    NSLog(@"video producer local id:%@", producer.localId);
...
-(void)closeAudioProducer {
    ...
    [self request:@"closeProducer"
         jsonData:data
        onSuccess:^(ProtooclientResponse *r) {
        NSLog(@"close audio producer success, id");
    }
          onError:nil];
    self.localAudioTrack = nil;
    self.audioSource = nil;
...
-(WebRTCVideoView*)createVideoView:(NSString*)id_ isLocal:(BOOL)isLocal {
    return [self.videoRendererDelegate createVideoView:id_ isLocal:isLocal];
}

-(void)removeVideoView:(NSString*)id_ {
    [self.videoRendererDelegate removeVideoView:id_];
}
...
- (void)onNotification:(ProtooclientNotification* _Nullable)p0 {
    ...
    if ([method isEqualToString:@"newPeer"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(roomClient:peerJoined:)]) {
                [self.delegate roomClient:self peerJoined:object];
            }
        });
